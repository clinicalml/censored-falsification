...
class DataModule:
    ...
    def __init__(self, confounder_seed, noise_seed, **data_cfg):
        ...
        num_obs_datasets = data_cfg['obs_dict']['num_obs']
        ...
        # variable init
        self.num_continuous  = data_cfg['num_continuous']
        self.num_binary      = data_cfg['num_binary']
        ...
        self.oracle_params = {"selection_model": {"P_X_S0": None, "P_X_S1": None, "P_S0": None},
                            "response_surface": {"beta_B": None, "gamma": None, "omega": None, "W": None}}
    ...
    def _simulate_outcomes(self,
        ...
        c0 = []; c1 = [] # true
        c  = [] # noise
        d = [] # censoring indicator (1 if observed, 0 if censored)
...
#         np.random.seed(self.noise_seed)
        ...
        for idx, row in confound_table.iterrows(): 
            ...
            if self.censoring_type == 'cond_indep':
                censoring_shift = 0.5
            ...
            c_noise = np.random.normal(row['treat']*c_mean1 + (1-row['treat'])*c_mean0, scale=1.)
             
            # y_noise = np.random.normal(row['treat']*mean1 + (1-row['treat'])*mean0, scale=.01) # for FIG1 (.01 scale)
            y.append(y_noise)
            c.append(c_noise)
            d.append(int(c_noise >= y_noise))
        ...
        confound_table.insert(loc=0, column=f'c0_{data_type}', value=c0)
        confound_table.insert(loc=0, column=f'c1_{data_type}', value=c1)
        confound_table.insert(loc=0, column=f'c_{data_type}', value=c)
        
        confound_table.insert(loc=0, column=f'd_{data_type}', value=d)
